from flask import Flask, render_template, request, redirect, url_for, Blueprint, session, send_from_directory, abort, flash
    ...
    if 'archivos' in request.files:
        ...
        for archivo in archivos:
            ...
            if archivo and '.' in archivo.filename and archivo.filename.rsplit('.', 1)[1].lower() in {'pdf', 'doc', 'docx', 'xls', 'xlsx'}:
                ...
                nombre = f"{nuevo_requerimiento.id}_{tipo}_{original_filename}"
                archivo.save(os.path.join(upload_folder, nombre))  # Guarda el archivo

                # Crear Archivo
                nuevo_archivo = Archivo(nombre, tipo, nuevo_requerimiento.id)
...
def misSolicitudes():
    ...
    for req in requerimientos:
        req.archivos = Archivo.query.filter(Archivo.idTipo == req.id, Archivo.tipo == "requerimiento").all()
    
    for com in comentarios:
        com.archivos = Archivo.query.filter(Archivo.idTipo == com.id, Archivo.tipo == "comentario").all()
...
def registrarCategoriaRequerimiento():
    ...
    catIgual = CategoriaRequerimiento.query.filter_by(idTipo=idTipo, descripcion=descripcion).first()
    if catIgual:
        flash("Los datos ingresados ya existen", "error")
...
def registrarTipoRequerimiento():
    ...
    tipoXtipo = TipoRequerimiento.query.filter_by(tipo=tipo).first()
    if tipoXtipo:
        flash("Los datos ingresados ya existen", "error")
        return redirect(url_for('tipoRequerimiento.indexTipoRequerimiento'))
    tipoXcodigo = TipoRequerimiento.query.filter_by(codigo=codigo).first()
    if tipoXcodigo:
        flash("Los datos ingresados ya existen", "error")
        return redirect(url_for('tipoRequerimiento.indexTipoRequerimiento'))
...
def registrarComentario():
    ...
    if 'archivos' in request.files:
        ...
        for archivo in archivos:
            ...
            if archivo and '.' in archivo.filename and archivo.filename.rsplit('.', 1)[1].lower() in {'pdf', 'doc', 'docx', 'xls', 'xlsx'}:
                ...
                original_filename = secure_filename(archivo.filename)
                # Crear un nuevo nombre con el ID del requerimiento
                nombre = f"{idComentario}_{tipo}_{original_filename}"
                archivo.save(os.path.join(upload_folder, nombre))  # Guarda el archivo
                

                # Crear Archivo
                nuevo_archivo = Archivo(nombre, tipo, idComentario)
                db.session.add(nuevo_archivo)
    ...
    elif request.form['ubicacion'] == "verSolicitudes":
        return redirect(url_for('requerimiento.verSolicitudes'))


# Define el blueprint para ARCHIVO
#archivo = Blueprint('archivo', __name__)
archivo = Blueprint('archivo', __name__, static_folder='statics', static_url_path='/statics')
...
@archivo.route('/verArchivo/<path:nombre_archivo>')
def servir_archivo(nombre_archivo):
    carpeta_archivos = os.path.join(os.getcwd(), 'statics', 'img', 'archivos')

    # Verificar si el archivo existe antes de enviarlo
    archivo_path = os.path.join(carpeta_archivos, nombre_archivo)
    if not os.path.isfile(archivo_path):
        abort(404)  # Devuelve un error 404 si el archivo no existe

    return send_from_directory(carpeta_archivos, nombre_archivo)