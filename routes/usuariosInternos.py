from flask import Flask, redirect, Blueprint, request, url_for, session, flash, jsonify
...
@usuariosInternos.route('/usuario/regInt', methods=['POST'])
def RegInterno():
    data = request.json  # Recibe el JSON
    ...
    departamento = data.get('departamento')  # Ahora sí leerá el departamento
    ...
    return jsonify({"mensaje": "Cargado con éxito"})
...
def RegistrarInterno():
    ...
    interno = UsuarioInterno.query.filter_by(legajo=legajo).first()
    if interno:
        flash("Legajo existente. Por favor, inténtalo de nuevo.", "danger")
        return redirect(url_for('auth.indexUsuarioInterno'))

    # Verificar si el usuario ya existe
    interno = UsuarioInterno.query.filter_by(usuario=usuario).first()
    if interno:
        flash("Usuario existente. Por favor, inténtalo de nuevo.", "danger")
        return redirect(url_for('auth.indexUsuarioInterno'))

    # Verificar si el correo ya existe
    interno = UsuarioInterno.query.filter_by(correo=correo).first()
    if interno:
